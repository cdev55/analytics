  ...
  sendMessage = (messageObject, delaySeconds = null) => {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        this.sqs.sendMessage(delaySeconds ? { ...params, DelaySeconds: delaySeconds } : { ...params }, (err, data) => {
          if (err) {
            console.log(`something went wrong, while writing SQS ERROR: ${err}`)
            return reject(err)
          };
          if (!err && data) {
        ...
        console.log(`something went wrong while writing SQS! - ERROR: ${error.message}`)
        return reject(error)
      }
    })
  }
  ...
  deleteMessage = (queueId) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        console.log(`something went wring while deleting messsage: ${error.messsage}`)
        return reject(error)
      }
    })
  }
